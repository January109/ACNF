...
Before considering a general series of the form $\sum_{m = 1}^\infty a_mm^{-s}$, we first establish these convergence results for the Riemann zeta function $\zeta(s) = \sum_{m = 1}^\infty m^{-s}$, which will correspond to the principal term in our asymptotic formula for the ideals of bounded norm, and the pole of $\zeta_K$ at $s = 1$.
\begin{lemma}
...
\end{lemma}
\begin{proof}
    Let $\Re(s) > \sigma$. Applying Abel's theorem [\hyperref{apostol}{1}, Theorem 4.2] to $f(x) = x^{-s}$ and $A(x) = \sum_{m \leq x}a_m$ on $[1/2, x]$ and noting that $A(x) = 0$ for $x < 1$, we find
    $$
